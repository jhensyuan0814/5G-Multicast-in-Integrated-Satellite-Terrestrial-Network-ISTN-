...
side_collection = [3000:2000:15000];%: 1000: 6000;
version_collection = [1:3];
debug = false; %you want to print the information about SFN during the algorithm
final_result = zeros(length(side_collection),length(version_collection)); %the final scores
scenario = 2;

for CM = 1:length(side_collection)
    for VER = 1:length(version_collection)
        version = version_collection(VER); %1:ours; 2:all SFN; 3:all PTM
        %parameter
        bandwidth = 10 ^ 7;
        side = side_collection(CM); %length of square boundary 
        BSNum = 19; %total number of BSs
        MUNum = 1000; %total number of MUs
       ...
       %global variable
        ...
        ini_BSMU_hex(side,BSNum,MUNum,scenario); %side = ISD
        %plot_BSMU(BSNum);
...
hfig = figure(1);
plot(side_collection, final_result(1, :), '--o','LineWidth', 2);
hold on;
plot(side_collection, final_result(2, :), '--d','LineWidth', 2);
hold on;
plot(side_collection, final_result(3, :), '--x','LineWidth', 2);
hold off;
xlabel('Side length (m)');
ylabel('QoE score');
set(gca,'FontSize',16);
legend('Proposed', 'All SFN', 'All PTM', 'Location','northwest','FontSize',12);
topic = sprintf("Score for side scenario %d",scenario);
title(topic);  
saveas(gcf,'pic_capacity/'+topic+'.jpg');
grid on;