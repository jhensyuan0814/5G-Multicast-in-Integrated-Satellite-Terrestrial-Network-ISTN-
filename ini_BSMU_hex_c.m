function ini_BSMU_hex(R,BSNum,MUNum,scenario)
    ...
    h = R*sqrt(3)/2;
    side = R/sqrt(3);
    center_x = cat(2,0, R*cos(pi*[1/6:1/3:11/6]),sqrt(3)*R,sqrt(3)*R,sqrt(3)*R/2,0,-sqrt(3)*R/2,-sqrt(3)*R,-sqrt(3)*R,-sqrt(3)*R,-sqrt(3)*R/2,0,sqrt(3)*R/2,sqrt(3)*R);
    ...
    elseif scenario == 1 %Fixed density with uniform distribution
        MU = R * (complex(rand(1, MUNum*3)-0.5, rand(1, MUNum*3) - 0.5));
        IN = zeros(1,3*MUNum);
        temp_vx = side* cos((0:6)*pi/3) ; 
        temp_vy = side* sin((0:6)*pi/3) ; 
        IN = IN | inpolygon(real(MU), imag(MU), temp_vx, temp_vy);
        index = randperm(MUNum);
        MU = MU(IN);
        MU = MU(index);
    ...
    elseif scenario == 2 %Fixed density with clustered distribution
        MU = 0.25 * R * (complex(rand(1, MUNum*3)-0.5, rand(1, MUNum*3) - 0.5));
        IN = zeros(1,3*MUNum);
        temp_vx = 0.25*side* cos((0:6)*pi/3) ; 
        temp_vy = 0.25*side* sin((0:6)*pi/3) ; 
        IN = IN | inpolygon(real(MU), imag(MU), temp_vx, temp_vy);
        index = randperm(MUNum);
        MU = MU(IN);
        MU = MU(index);
        tv1 = BS + 0.6 * R * (complex(rand(1, BSNum)-0.5, rand(1, BSNum) - 0.5));
